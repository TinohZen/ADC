...
export default function ConfirmModal({ isOpen, title, message, confirmText = "OK", type = 'info', onConfirm, onCancel, loading, onlyConfirm }: ConfirmModalProps) {
  const colors = {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 text-center border border-white">
            <div className={`w-20 h-20 rounded-[2rem] mx-auto mb-6 flex items-center justify-center shadow-inner ${type === 'danger' ? 'bg-red-50 text-red-500' : type === 'success' ? 'bg-emerald-50 text-emerald-500' : 'bg-blue-50 text-blue-500'}`}>
            ...
            <div className="flex gap-3">
              {!onlyConfirm && (
                <button onClick={onCancel} className="flex-1 py-4 rounded-2xl bg-slate-100 text-slate-500 font-bold text-xs hover:bg-slate-200 transition-all uppercase tracking-widest">Annuler</button>
              )}
              <button onClick={onConfirm || onCancel} disabled={loading} className={`flex-1 py-4 rounded-2xl font-bold text-xs shadow-lg transition-all flex items-center justify-center gap-2 uppercase tracking-widest disabled:opacity-50 ${colors[type]}`}>
