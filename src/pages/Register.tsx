...
import { User, Phone, Mail, Lock, Camera, CheckCircle2, Eye, EyeOff, Loader2, AlertCircle } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(d.error || "Erreur d'inscription"); 
      }
    } catch(err: any) { 
  ...
  if (status.success) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-10 sm:p-12 rounded-[2.5rem] shadow-2xl text-center max-w-md border border-slate-100">
          <CheckCircle2 size={56} className="text-emerald-500 mx-auto mb-4" />
          <h2 className="text-2xl font-black text-slate-800 mb-2">INSCRIPTION RÉUSSIE</h2>
          <p className="text-slate-500 text-xs sm:text-sm mb-6">Un administrateur doit valider votre compte avant votre première connexion.</p>
          <Link to="/login" className="block w-full bg-emerald-600 text-white py-3.5 rounded-2xl font-bold text-xs uppercase tracking-wider">
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 py-8 font-sans">
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white p-6 sm:p-10 rounded-[2.5rem] shadow-2xl w-full max-w-lg border border-white">
        <div className="text-center mb-8">
          <img src="/logoADC.png" alt="Logo" className="w-20 h-20 mx-auto mb-2 object-contain" onError={(e) => { e.currentTarget.style.display = 'none'; document.getElementById('logo-fallback-reg')!.style.display = 'flex'; }} />
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-slate-800">Rejoindre l'ADC</h1>
          <p className="text-slate-400 font-bold text-[9px] uppercase tracking-widest mt-1">Association Madagascar</p>
        </div>

        {status.error && (
          <div className="bg-red-50 text-red-600 p-3.5 rounded-2xl mb-5 text-xs font-bold border border-red-100 flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div className="flex justify-center mb-4">
            <label className="relative cursor-pointer group">
              <div className="w-20 h-20 rounded-[1.8rem] bg-slate-50 border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden group-hover:border-emerald-500 transition-all">
                {formData.photo_url ? <img src={formData.photo_url} className="w-full h-full object-cover" /> : <Camera size={28} className="text-slate-300" />}
              </div>
          ...
          <div className="relative">
            ...
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 bottom-3 text-slate-300">
              {showPassword ? <EyeOff size={16}/> : <Eye size={16}/>}
            </button>
          </div>

          <button type="submit" disabled={status.loading} className="w-full bg-slate-900 text-white p-4 rounded-2xl font-black text-xs tracking-widest shadow-xl hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 cursor-pointer mt-3">
            {status.loading && <Loader2 size={16} className="animate-spin"/>} S'INSCRIRE MAINTENANT
          </button>
        </form>
        <p className="text-center mt-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Déjà membre ? <Link to="/login" className="text-emerald-600 hover:underline font-extrabold">Se connecter</Link></p>
      </motion.div>
    </div>
  );
}
...
const InputReg = ({ label, ...props }: any) => (
  <div className="flex flex-col gap-1">
    ...
    <input {...props} className="w-full p-3 bg-slate-50 border border-transparent rounded-xl text-xs font-bold outline-none focus:bg-white focus:border-emerald-500 transition-all" required />
  </div>
);
...
const SelectReg = ({ label, value, options, onChange, name }: any) => (
  <div className="flex flex-col gap-1">
    <label className="text-[9px] font-bold text-slate-400 uppercase ml-2 tracking-widest">{label}</label>
    <select name={name} value={value} onChange={onChange} className="w-full p-3 bg-slate-50 border-none rounded-xl text-xs font-bold outline-none focus:bg-white transition-all cursor-pointer">
